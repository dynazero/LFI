...
function materialsLogsUpdate(){
		...
		var formData = $("#userEncoder").val();
			
		//var formData = val;
		
    	//var formData = this.value;
	
	 $.post("phpscripts/Parse_MyMaterialsLogsUpdate2.php",{MyData:formData},function(result){
...
function minimumChecker(){
    ...
    <h3 class="page-header">Inventory <small>Materials Add</small></h3> 
  ...
  <div class="rightMatADD"> 
		...
		while($row=mysqli_fetch_array($result)){?>	           
           ...
           <select class="form-control" autofocus id="projectsMatAddName" name="projectsMatAddName" onChange="projectMatCheck()"> 
            ...
            <?php $result=mysqli_query($con,"SELECT `project`.`View` AS State, `project`.`Activated` AS ProjectStatus, `project`.`adminOwner` AS adminOwner,`project`.`Employees` AS Employees,`project`.`ProjectName` AS ProjectName, `project`.`projectID` AS projectID, `users`.`UserName` AS UserName,`project`.`filePath` AS filePath FROM `project` JOIN `users` ON `project`.userID = `users`.usersID WHERE `project`.Activated IN (1,2) AND `project`.View IN (2,3) AND `project`.adminOwner = $userID ORDER BY `project`.projectID DESC");
		while($row=mysqli_fetch_array($result)){?>	           
            ...
            <input type="hidden" name="userEncoder" id="userEncoder" value="<?php echo $_SESSION['usersID'];?>"/>
